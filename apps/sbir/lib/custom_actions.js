/**
 The Sbir.CustomActions mixin allows actions and custom mouse events for any
 ...
 {{{
 SC.View.extend(Sbir.classActions, {

 actions: {
   "#my-search": {  // Match on ID
     target: 'App.globalSearchRecord',
     action: 'show'
   },
   // Or match on Class
   ".help":   {target: 'App.helpController', action: 'show'},
   ".logout": {
     action: 'logout',
...
Sbir.CustomActions = {
  ...
  _setDetails:function (classDetails) {
    if (classDetails.set) {
      classDetails.set('layer', this);
    }
    else {
      classDetails['layer'] = this;
    }
  },
